...
def require_api_auth(fn):
    async def _wrapped(*args, **kwargs):
        ...
        sig = request.headers.get("X-API-Signature", "").strip()
        ts = request.headers.get("X-API-Timestamp", "").strip()
        nonce = request.headers.get("X-API-Nonce", "").strip()
        ...
        with sqlite3.connect(DB_FILE) as db:
            row = db.execute("SELECT user_id, secret_encrypted, revoked FROM api_keys WHERE key_id = ?", (key_id,)).fetchone()
            if not row:
                return jsonify({"error": "Invalid API key."}), 401
            user_id, secret_encrypted, revoked = int(row[0]), row[1], int(row[2] or 0)
            if revoked:
                return jsonify({"error": "API key revoked."}), 401

            if not _api_check_and_store_nonce(db, key_id, nonce, ts_int):
                return jsonify({"error": "Replay detected."}), 401

            try:
                secret = decrypt_data(secret_encrypted)
            except Exception:
                logger.exception("API secret decrypt failed")
                return jsonify({"error": "Server key error."}), 500

            expected = _api_sign(secret, canonical)
            if not hmac.compare_digest(expected, sig):
                return jsonify({"error": "Bad signature."}), 401

            ok, msg = _api_check_quota(db, user_id)
            if not ok:
                return jsonify({"error": msg}), 402

            # attach user_id for downstream
            request.api_user_id = user_id  # type: ignore
            request.api_key_id = key_id    # type: ignore
...
def send_email(to_addr: str, subject: str, text_body: str, html_body: str | None = None) -> bool:
    ...
    if _internal_mail_enabled():
        try:
            ...
            mailer.enqueue(_dkim_sign_email_message(msg), to_addr)
            return True
        except Exception:
...
def create_tables():
    ...
    with sqlite3.connect(DB_FILE) as db:
        ...
        # Ensure uniqueness for migrated DBs with an index instead.
        cursor.execute(
...
<div class="container" style="max-width:560px;">
  ...
  <form method="POST" novalidate>
    ...
    <div class="form-group mt-2">
      ...
      <small class="form-text">12-256 characters with uppercase, lowercase, and a number.</small>
    ...
    if request.method == 'POST':
        ...
        if not error:
            ...
            with sqlite3.connect(DB_FILE) as db:
                ...
                if not row:
                    msg = "If the account exists, a reset link will be issued."
                else:
                    user_id = int(row[0])
                    if _user_has_google_sub(db, user_id):
                        ...
                        # Optional email delivery. If SMTP is not configured, show token once.
                        ...
                        if smtp_host and smtp_from:
                            try:
                                ...
                                if to_addr:
                                    ...
                                    em["From"] = smtp_from
                                    em["To"] = to_addr
                                    em["Subject"] = RESET_EMAIL_SUBJECT
                                    em.set_content(RESET_EMAIL_TEXT.format(reset_url=reset_url))
                                    with smtplib.SMTP(smtp_host, int(os.getenv("SMTP_PORT","587"))) as smtp:
                                        smtp.starttls()
                                        if smtp_user and smtp_pass:
                                            smtp.login(smtp_user, smtp_pass)
                                        smtp.send_message(em)
                                    msg = "If the account exists, a reset link was sent."
                                else:
                                    msg = f"Reset link (displayed once): {reset_url}"
                            except Exception:
...
<div class="container" style="max-width:520px;">
  ...
  <form method="POST">
    ...
    <div class="form-group mt-2">
      ...
      <input class="form-control" name="email" autocomplete="email">
    </div>
    {{ captcha_widget|safe }}
    <button class="btn btn-primary mt-3" type="submit">Issue reset link</button>
    <a class="btn btn-link mt-3" href="{{ url_for('login') }}">Back to login</a>
  </form>
</div>
</body></html>
""", error=error, msg=msg, captcha_widget=captcha_widget)
...
def api_create_key():
    ...
    with sqlite3.connect(DB_FILE) as _cdb2:
        _api_cache_set(
            _cdb2,
            user_id=user_id,
...
async def api_scan():
    ...
    with sqlite3.connect(DB_FILE) as _cdb:
