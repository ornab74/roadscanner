...
def _enforce_origin_checks():
    ...
    Exempts API HMAC callers and Stripe webhooks.
    """
    ...
    # Determine expected origin from request host (supports ProxyFix if enabled)
    expected = request.host_url.rstrip("/")

    def _matches(v: str) -> bool:
        return v.startswith(expected)

    # Browsers typically send Origin on POST; if missing, fall back to Referer.
    if origin:
        ...
        if not referer:
            # Fail closed for browser sessions (prevents CSRF in unusual contexts)
...
<div class="d-flex mt-2" style="gap:10px;">
  <button class="btn btn-outline-light btn-sm" type="button" id="toggleLoginPwd">Show</button>
</div>
<small class="form-text" style="opacity:.85;">{{ pw_req_text }}</small>
    </div>
    {{ captcha_widget|safe }}
  ...
  {% if google_auth_available %}
    <a class="btn btn-light w-100 mt-3" href="{{ url_for('google_start') }}">Continue with Google</a>
  {% endif %}
...
""", form=form, error_message=error_message, registration_enabled=registration_enabled,
       google_auth_available=google_auth_available, captcha_widget=captcha_widget)
...
<div class="container" style="max-width:560px;">
  ...
  <form method="POST" novalidate>
    ...
    <div class="form-group mt-2">
      {{ form.password.label }}
      {{ form.password(class="form-control", placeholder="Choose a password", id="regPassword") }}
      <small id="regPwReq" class="form-text">{{ pw_req_text }}</small>
      <div class="d-flex mt-2" style="gap:10px;">
        <button class="btn btn-outline-secondary btn-sm" type="button" id="genPwdBtn">Create strong password</button>
        <button class="btn btn-outline-secondary btn-sm" type="button" id="copyPwdBtn">Copy</button>
        <button class="btn btn-outline-secondary btn-sm" type="button" id="toggleRegPwd">Show</button>
      </div>
    </div>
  ...
  {% if google_auth_available %}
    <a class="btn btn-light w-100 mt-3" href="{{ url_for('google_start') }}">Register / Login with Google</a>
  {% endif %}
...
(function(){
  ...
  function gen(){
    ...
    out += pick(upper) + pick(lower) + pick(nums) + pick(sym);
  ...
  if(copyBtn) copyBtn.addEventListener('click', async ()=>{ try{ await navigator.clipboard.writeText(pwd.value||""); }catch(e){} });
})();
</script>
</body>
</html>
...
<head>
    ...
    <style>body {
        ...
        .btn-custom {
            background: linear-gradient(135deg, #1e3c72 0%, #2a5298 60%, #3b7bdc 100%);
        ...
        .btn-custom:hover {
            ...
            box-shadow: 0 14px 30px rgba(0,0,0,0.26), inset 0 1px 0 rgba(255,255,255,0.25);
        }
        .btn-custom:active {
            transform: translateY(0);
            filter: brightness(0.98);
            box-shadow: 0 8px 18px rgba(0,0,0,0.22), inset 0 1px 0 rgba(255,255,255,0.18);
        }
.btn-custom:hover {
            background: #019a9e;
        }
...
<body>
    ...
    <div class="sidebar">
        <div class="navbar-brand">QRS</div>
        <a href="#" class="nav-link active" onclick="showSection('step1')"><span>Dashboard</span>
        </a>
        {% if session.is_admin %}
        <a href="{{ url_for('settings') }}"><span>Settings</span>
        </a>
        <a href="{{ url_for('admin_blog_backup_page') }}"><span>Blog Backup</span>
        </a>
        <a href="{{ url_for('admin_local_llm_page') }}"><span>Local Llama</span>
        </a>
        {% endif %}
        <a href="{{ url_for('logout') }}"><span>Logout</span>
        </a>
    </div>
    ...
    <div class="content">
        <div class="stepper">
            <div class="step active" id="stepper1">
                <div class="circle">1</div>Grabs
            </div>
            <div class="step" id="stepper2">
                <div class="circle">2</div>Street Name
            </div>
            <div class="step" id="stepper3">
                <div class="circle">3</div>Run Scan
            </div>
        </div>
        ...
        <div id="step1" class="form-section active">
            <form id="grabCoordinatesForm">
                ...
                <button type="button" class="btn btn-custom" onclick="getCoordinates()">Get Current Location
                </button>
                <button type="button" class="btn btn-custom" onclick="nextStep(1)">Next
                </button>
            </form>
        ...
        <div id="step2" class="form-section">
            ...
            <button type="button" class="btn btn-custom" onclick="nextStep(2)">Next
            </button>
        </div>
...
{% endif %}
                ...
                <button type="button" class="btn btn-custom" onclick="startScan()">Start Scan
                </button>
            </form>
            ...
            <table class="table table-dark table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn btn-info btn-sm" onclick="viewReport({{ report['id'] }})">View
                            </button>
                        </td>
                    </tr>
                    {% endfor %}
    ...
    <script>var csrf_token = {{ csrf_token | tojson }};
        ...
        function prependReportToTable(reportId, timestamp) {
            const newRow = `
                <tr>
                    <td>${timestamp}</td>
                    <td>
                        <button class="btn btn-info btn-sm" onclick="viewReport(${reportId})">View
                        </button>
                    </td>
                </tr>
            `;
...
(function(){
    ...
    if(copyBtn) copyBtn.addEventListener('click', async ()=>{
        try{ await navigator.clipboard.writeText(pwd.value||""); }catch(e){}
    });
})();
...
def api_create_key():
    ...
    # Store cache entry after a successful scan
